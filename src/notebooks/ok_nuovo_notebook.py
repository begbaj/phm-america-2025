...
for i in range(0, 48):
    ...
    for esn in data["ESN"].unique():
        ...
        axs[0].plot(hi_hpt_final.rolling(window=1,min_periods=1).mean(), color='tab:blue', label='Predicted HPT RUL')
        axs[0].legend(fontsize=28)
        axs[1].plot(hi_hpc_final.rolling(window=1,min_periods=1).mean(), color='tab:green', label='Predicted HPC RUL')
