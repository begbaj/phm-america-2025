...
from predictor.data_loading import DataLoading
from predictor.hi_trainer import HITrainer
from predictor.lgbm_classifier import LGBMCycleClassifier
from predictor.lgbm_gap_correction import LGBMGapCorrection
from predictor.ww_trainer import WWTrainer
...
def main() -> None:
    ...
    results_df = pd.concat(
        [results_val, results_test], ignore_index=True
    )
    ...
    for i, engine_df in enumerate(data.test):
        ...
        if mask.any():
            cycles_hpt = float(
                results_test.loc[mask, "Cycles_to_HPT_SV"].values[0]
            )
            cycles_hpc = float(
                results_test.loc[mask, "Cycles_to_HPC_SV"].values[0]
            )
        else:
            cycles_hpt = _fallback_hpt
            cycles_hpc = _fallback_hpc
            print(
                f"  {file_name}: FALLBACK HPT/HPC "
                f"(ESN {esn} not found in results_test)"
            )

        # --- WW ---
        if esn in ww_results_test_final:
            cycles_ww = ww.cycles_to_next_ww_from_end(
                ww_results_test_final[esn]
            )
        elif i in ww_results_test_final:
            cycles_ww = ww.cycles_to_next_ww_from_end(
                ww_results_test_final[i]
            )
        else:
            cycles_ww = 0
