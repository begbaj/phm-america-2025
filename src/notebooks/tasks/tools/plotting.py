...
import numpy as np
import pandas as pd
from . import utils as u, config as cfg
from pandas import DataFrame
from matplotlib.figure import Figure
from typing import overload
from .types.plotdata import PlotData
from .types.enums import ESENSORS, RepairEventType
import os
...
def plot_stat_ess(data: DataFrame):
    ...
    for esn in u.ESN:
        fig, axes = plt.subplots(
            nrows=4, ncols=4, figsize=(16, 12), sharey='row')
        ...
        for i, sensor in enumerate(u.SENSORS):
            ...
            for j, snapshot in enumerate(u.SNAPSHOTS):
                ddict[j] = u.df_ess_filter(
                    data, esn, sensor, snapshot).values.squeeze()
...
def plot_stat_feat_individually_mean(data: dict, pdata: PlotData, repair: RepairEventType, stop=True, featlist: list = None, save: bool = False, show=True):
    ...
    for ax, feat in zip(axes, featlist):
        ...
        for label, y_data in series_data:
            ...
            # 2. Plot the raw data
            ax.plot(x_vals, y_plot, label=label,
                    color=ccol, linewidth=1, alpha=0.1)
            ...
            for degree, color, style, alpha, width in trends:
                ...
                y_smooth = pd.Series(y_plot).rolling(
                    window=window, center=True, min_periods=1).std().to_numpy()
                ax.plot(x_vals, y_smooth, color=ccol,
                        linestyle=style, alpha=alpha, linewidth=width)
                y_smooth = pd.Series(y_plot).rolling(
                    window=window, center=True, min_periods=1).mean().to_numpy()
                ax.plot(x_vals, y_smooth, color=ccol, linestyle=style,
                        alpha=alpha-0.05, linewidth=width-0.5)
    ...
    if save:
        ...
        path = u.plot_path("STAT_FEATURES", pdata.repair,
                           pdata.esn, pdata.sensor, filename=filename)
    ...
    for ax, feat in zip(axes, featlist):
        ...
        for label, y_data in series_data:
            ...
            for degree, color, style, alpha, width in trends:
                ...
                p = np.poly1d(z)
                ax.plot(x_trend, p(x_trend), color=ccol,
                        linestyle=style, alpha=alpha, linewidth=width)
...
def plot_features(dff: DataFrame, esn_list: list[int], tot: DataFrame, target: str, fulltarget: str, filter_feature: str | None = None, max_features_to_show: int = 6):
    ...
    for esn in esn_list:
        ...
        fig, axes = plt.subplots(
            n_rows, n_cols, figsize=(40, 6 * n_rows), squeeze=False)
        ...
        for i, feat_name in enumerate(target_features):
            ...
            if fault_col in esn_data.columns:
                for f_idx in esn_data.loc[esn_data[fault_col] == 1, 'esn_index']:
                    ax1.axvline(x=f_idx, color='red', linestyle=':',
                                alpha=0.8, label='Fault')
...
def plot_features_per_snap(dff: DataFrame, esn_list: list[int], tot: DataFrame, target: str, fulltarget: str, filter_feature: str | None = None, max_features_per_snap: int = 6):
    ...
    for esn in esn_list:
        ...
        for snap in sorted(esn_all_data['snap'].unique()):
            group_data = esn_all_data[esn_all_data['snap']
                == snap].sort_values('esn_index')

            # 1. Selezione Feature per questo snap specifico
            if filter_feature:
                snap_best = tot[(tot['snap'] == snap) & (
                    tot['feature'] == filter_feature)]
            else:
                # Prende le migliori N feature per questo snap basandosi sulla correlazione totale
                snap_best = tot[tot['snap'] == snap].sort_values(
                    'tot_val', key=abs, ascending=False).head(max_features_per_snap)
            ...
            fig, axes = plt.subplots(n_rows, n_cols, figsize=(
                n_cols * 10, 6 * n_rows), squeeze=False)
            ...
            for i, feat_name in enumerate(target_features):
                ...
                ax1.set_ylabel('Feature Value',
                               color=color_feat, fontweight='bold')
                ...
                ax2.set_ylabel(f'To Next {target}',
                               color=color_rul, fontweight='bold')
                ax2.tick_params(axis='y', labelcolor=color_rul)

                # LINEE DI FAULT (Verticali)
                fault_col = f'fault_{target.lower()}_cycle'
                if fault_col in group_data.columns:
                    faults = group_data.loc[group_data[fault_col]
                        == 1, 'esn_index']
                    for f_idx in faults:
                        ax1.axvline(x=f_idx, color='red',
...
def plot_pipeline_comparison(history, sensor_name):
    ...
    for i, step in enumerate(steps):
        ...
        if i == boxplot_index:
            ...
            bp = ax.boxplot([data_prev, data_current],
                            vert=True, patch_artist=True, widths=0.5)

            # Coloriamo i due box in modo diverso per distinguerli
            # Colore dello step precedente
            bp['boxes'][0].set(facecolor=colors[i-1], alpha=0.5)
            # Colore dello step attuale
            bp['boxes'][1].set(facecolor=colors[i], alpha=0.8)
            ...
            ax.set_xticks([1, 2])
            ax.set_xticklabels(
                [f"BEFORE ({prev_step})", f"AFTER ({step})"], fontweight='bold')
            ax.set_ylabel("Value Range")
            ax.set_title(f"STEP {i}: OUTLIERS REMOVAL",
                         fontsize=14, fontweight='bold', loc='left')

        # --- LOGICA SCATTER PER GLI ALTRI STEP ---
        else:
            data = history[step][sensor_name]
            ax.scatter(data.index, data, s=2, label=step,
                       color=colors[i], alpha=0.6)
            ax.plot(data.index, data,
                    color=colors[i], alpha=0.2, linewidth=0.5)

            ax.set_title(f"STEP {i}: {step.upper()}",
                         fontsize=14, fontweight='bold', loc='left')
...
def plot_residuals_dashboard(df, residual_cols):
    ...
    for res_col in residual_cols:
        fig, axes = plt.subplots(
            rows, cols, figsize=(18, 5 * rows), sharex=False)
...
def plot_engine_level_residuals(df, residual_cols, to_next_col, event):
    ...
    for res_col in residual_cols:
        ...
        fig.suptitle(
            f'Engine-Level Residuals: {res_col} - event: {event}', fontsize=22, y=1.02)
...
def plot_feature_importance(importances: pd.Series):
    """
    Genera un plot dell'importanza delle feature.
    """
    plt.figure(figsize=(10, 6))
    plt.title('Feature Importances')
    plt.barh(range(len(importances)), importances.values, color='b', align='center')
