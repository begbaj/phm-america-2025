...
#Serie filtarte con Kalman
filtered_data = None
with open("../../Data/PHM2025_training_data_Kalman.csv", "r") as f:
...
# %%
# Filtro di Kalman per smoothing e stima dei NaN
...
for idx, sensor in enumerate(generals.SENSORS):
    sensor_name = sensor.value if hasattr(sensor, 'value') else sensor
    print(f"Elaborazione sensore: {sensor_name}")
    # Applichiamo la funzione per ogni gruppo di Motore e Snapshot
    filtered_data[sensor_name] = filtered_data.groupby(['ESN', 'Snapshot'])[sensor_name].transform(apply_kalman)

# Alla fine del ciclo for che crea filtered_data
# 1. Verifica finale: controlliamo se ci sono ancora NaN nei dati filtrati
for sensor in generals.SENSORS:
    sensor_name = sensor.value if hasattr(sensor, 'value') else sensor
    nan_residui = filtered_data[sensor_name].isna().sum()
    print(f"Processo completato. NaN residui nel sensore {sensor_name}: {nan_residui}")
# 2. Definisci il nome del file
output_dir = "../../Data"
final_save_path = f"{output_dir}/PHM2025_training_data_Kalman.csv"
# 3. Salvataggio in CSV
# index=False evita di aggiungere una colonna inutile per gli indici di riga
filtered_data.to_csv(final_save_path, index=False)
print(f"File salvato con successo come: {final_save_path}")


# %%
# Grafici di confronto per serie originali e filtrate

def plot_kalman_comparison(df_orig, df_filt, esn, snapshot):
    ...
    filt_sub = df_filt[(df_filt['ESN'] == esn) & (df_filt['Snapshot'] == snapshot)].sort_values('Cycles_Since_New')
    if orig_sub.empty:
    ...
    fig.suptitle(f'Dashboard Serie Filtrate - Motore: {esn} | Snapshot: {snapshot}', fontsize=20)    
    axes = axes.flatten()
    for i, sensor_name in enumerate(sensor_names):
        ax = axes[i]
        plot_data_orig = orig_sub[sensor_name].reset_index(drop=True)
        plot_data_filt = filt_sub[sensor_name].reset_index(drop=True)
        # Plot originale
        ax.plot(plot_data_orig.index, orig_sub[sensor_name], color='red', linestyle='--', alpha=0.3, linewidth=1, label='Original')
        # Plot Filtrato
        ax.plot(plot_data_filt.index, filt_sub[sensor_name], color='blue', linestyle='-', alpha=0.9, linewidth=1.5, label='Filtered')
        ax.set_title(f'{sensor_name}', fontsize=10)
        ax.grid(True, alpha=0.2)
        if i == 0: ax.legend() 
    plt.tight_layout(rect=[0, 0.03, 1, 0.95]) # Lascia spazio per il titolo globale
    # Salvataggio dashboard
    output_dir = f"img/KALMAN/ESN_{esn}"
    os.makedirs(output_dir, exist_ok=True)
...
for esn in generals.ESN:
    for snap in generals.Snapshot:
        plot_kalman_comparison(train, filtered_data, esn, snap)
